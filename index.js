...
const { handleIncomingImage } = require("./lib/media");
...
function clearUserState(username) {
  ...
  if (u) {
    ...
    console.log(`[${username}] User state cleared`);
  }
}
      ...
      if (u.qrPausedUntil && now < u.qrPausedUntil) {
        ...
        if (!u.lastQrAt || now - u.lastQrAt >= QR_DEBOUNCE_MS) {
          ...
          if (u.qrAttempts >= MAX_QR_ATTEMPTS) {
            ...
            console.warn(`[${username}] Too many QR attempts. Paused ${COOLDOWN_MS / 60000}m`);
          }
        }
      }
    }
      ...
      setTimeout(async () => {
        try {
          ...
          try {
            ...
            console.log(`[${username}] cleanCategories: kept=${res.kept} fixed=${res.fixed} dropped=${res.dropped}`);
          } catch (e) {
    ...
    if (connection === "close") {
      ...
      switch (code) {
        ...
        default:
          ...
          setTimeout(() => startUserSession(username).catch(() => {}), 5000 + Math.random() * 10000);
      }
    }
  });
  ...
  sock.ev.on("messages.upsert", async ({ messages }) => {
    try {
      ...
      for (const msg of messages || []) {
        ...
        if (!fromMe && chatBare !== selfBare) continue;
        if (fromMe && chatBare !== selfBare) continue;
        u.ownerJid = selfBare;
        ...
        try {
          ...
          console.error(`[${username}] handleBroadcastMessage error:`, e.message);
        }
      }
    } catch (e) {
...
app.use("/schedule-job", require("./routes/schedule-job")(USERS));
...
app.post("/reset-qr/:username", (req, res) => {
  ...
  u.qrAttempts = 0;
  u.qrPausedUntil = 0;
  clearUserState(req.params.username);
  ...
  try {
    ...
    res.json({
      ...
      runtime: {
        ...
        ownerJid: u?.ownerJid,
        connecting: !!u?.connecting,
        ignoreIdsSize: u?.ignoreIds?.size || 0
      }
    });
  } catch (e) {
...
startScheduler(USERS);

console.log("[broadcast] build=fixed-message-handling DIAG/echo-detection-fixed");
