...
async function handleBroadcastMessage(username, msg, sock) {
  ...
  if (body === '/help') {
    const help = [
      ...
      '',
      `*Batching:* ${BATCH_SIZE} per batch, ${BATCH_INTERVAL_MS}ms interval. Set env BATCH_SIZE / BATCH_INTERVAL_MS.`,
      BATCH_STATUS_EVERY > 0 ? `*Updates:* every ${BATCH_STATUS_EVERY} batch(es) when loud.` : '*Updates:* final summary only (quiet).',
      '*Auto Mode:* send text or an image; I’ll infer the category and send. If unclear, I’ll use ALL.'
    ].join('\n');
  ...
  if (u.mode === 'text' && body && !body.startsWith('/' ) && !u.awaitingCategory) {
    ...
    if (AUTO_MODE) {
      ...
      await sendSys(username, sock, chatJid, { text: msg });
