...
const {
  ...
  prepareWAMessageMedia, // Baileys helper
      ...
      await Promise.all(slice.map(async (jid) => {
        try {
          if (messageContent.text !== undefined) {
            ...
            // ✅ Use prepared object the Baileys way: spread the prepared keys
            const caption = withFooter(baseCaption);
            await safeSend(jid, {
              ...waMediaPrepared, // (e.g. { imageMessage: ... }) Baileys maps this to a real image
              caption,
            ...
            const caption = withFooter(baseCaption);
            await safeSend(jid, { image: { url: imagePath }, caption }); // NO mimetype here
          } else {
