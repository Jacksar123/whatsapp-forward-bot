...
const {
  ...
  prepareWAMessageMedia, // ✅ Baileys function export (not sock method)
...
// Pre-upload once, free the original buffer/file ASAP; return WHOLE prepared object
async function prepareReusableImage(sock, imagePathOrBuffer) {
  ...
  return prepared; // e.g. { imageMessage: { ... } }
}
...
// ✅ 5-per-5s; inside each batch, send in parallel (so a batch doesn’t take 25s)
// ✅ Images: pre-upload once, unlink temp immediately, reuse prepared object
async function sendInBatches(sock, username, from, jids, messageContent, USERS) {
  ...
  let waMediaPrepared = null; // whole prepared object (e.g., { imageMessage: ... })
  ...
  if (messageContent.image?.url) {
    ...
    try {
      waMediaPrepared = await prepareReusableImage(sock, imagePathToUnlink);
    } catch (e) {
    ...
    while (idx < batch.length) {
      ...
      await Promise.all(slice.map(async (jid) => {
        try {
          ...
          } else if (waMediaPrepared) {
            const caption = withFooter(baseCaption);
            await safeSend(jid, {
              ...waMediaPrepared,       // << spread the prepared media
              caption,
...
async function handleBroadcastMessage(username, msg, USERS) {
  ...
  if (selection && u.lastPromptChat === from && u.awaitingCategory && (u.pendingImage || u.pendingText)) {
    ...
    if (u.pendingImage) {
      ...
      // Cleanup & reset (file unlinked during pre-upload step)
      u.pendingImage = null;
      u.lastPromptChat = null;
      return;
    }
