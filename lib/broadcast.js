...
// Safer pacing to avoid "Waiting for this message"
const BATCH_SIZE = 5;               // 5 groups per batch
const PARALLEL_PER_SLICE = 1;       // send to ONE group at a time inside the batch
const PER_SEND_DELAY_MS = 1000;     // 1s between sends to different groups
const BATCH_DELAY_MS = 5000;        // 5s between batches

// Per-send retry tuning
const SEND_MAX_RETRIES = 3;
const SEND_BACKOFF_MS = 1200;       // will exponential-backoff
...
/** robust per-jid sender with retries/backoff */
async function sendToOneWithRetry(sock, jid, payload) {
  ...
  while (attempt <= SEND_MAX_RETRIES) {
    try {
      ...
      const retryable =
        /timed out|rate|temporar|retry|disconnect|socket|stream|closed|request_entity_too_large|too many|econn|reset/i.test(msg);
      attempt++;
...
async function sendInBatches(sock, username, from, jids, messageContent) {
  ...
  // Track which groups we've "primed" during this broadcast
  ...
  while (sent < total) {
    ...
    // limited parallelism (actually 1 per slice by default)
    let idx = 0;
    while (idx < batch.length) {
      ...
      for (const jid of slice) {
        try {
          // PRIME: presence update + tiny wait to let sender keys propagate
          if (!primed.has(jid)) {
