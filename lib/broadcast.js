...
async function handleBroadcastMessage(username, msg, sock) {
  ...
  if (body.startsWith('/addcategory ')) {
    ...
    // Do NOT pop the picker here in Option B; user should type text first.
    return;
  }
  ...
  if (body === '/text') {
    ...
    // Option B: tell user to type message first (no picker yet)
  ...
  if (maybeNum && u.lastPromptChat === chatJid && u.awaitingCategory) {
    ...
    // If we already have content queued, send now
    ...
    // No content yet → store target and prompt for text (Option B)
  ...
  if (u.mode === 'text' && body && !body.startsWith('/')) {
    // User typed the message first → now show picker OR send to previously chosen target
    u.pendingText = body;
    u.awaitingPayload = null;
    u.pendingImage = null;
    u.lastPromptChat = chatJid;
  ...
  if (body && !body.startsWith('/')) {
    if (u.mode === 'text') {
      // handled in the text path above
      return;
