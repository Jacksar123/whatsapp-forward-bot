...
// New: light jitter + backoff caps to look human and avoid hammering
const JITTER_MS = 400;
const BACKOFF_START_MS = 60_000;      // start at 1m on throttle
const BACKOFF_MAX_MS   = 10 * 60_000; // max 10m
...
async function sendToOneWithRetry(sock, jid, payload) {
  ...
  while (attempt <= SEND_MAX_RETRIES) {
    try {
      ...
      // Special-case: "No sessions" appears during first attempts until sender keys land
      ...
      // Give keys time to propagate; warm group metadata helps too
...
async function sendInBatches(sock, username, from, jids, messageContent) {
  ...
  while (sent < total) {
    ...
    statusText += (sent + batch.length < total)
      ? `⏳ Next batch soon…`
