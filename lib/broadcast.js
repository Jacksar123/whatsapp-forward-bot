      ...
      await Promise.all(slice.map(async (jid) => {
        try {
          if (messageContent.text !== undefined) {
            ...
            await safeSend(jid, {
              ...waMediaPrepared, // spread prepared media (NO mimetype here)
              caption,
              contextInfo: { forwardingScore: 2, isForwarded: true }
            });
          } else if (messageContent.image?.url) {
            // fallback path (slower): per-send url upload (NO mimetype here!)
            const caption = withFooter(baseCaption);
...
async function handleBroadcastMessage(username, msg, USERS) {
  ...
  // numeric choice → do the broadcast
  ...
  // TEXT MODE: capture text payload first
  if (u.mode === 'text') {
  ...
  // MEDIA MODE: capture an image, then prompt
