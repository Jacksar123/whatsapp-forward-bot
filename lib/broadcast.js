...
async function sendInBatches(sock, username, from, jids, messageContent) {
  ...
  while (sent < total) {
    ...
    while (idx < batch.length) {
      const slice = batch.slice(idx, idx + 5);
...
/**
 * WhatsApp may show "Waiting for this message" while media keys propagate.
 * Retry and use updateMediaMessage to re-request keys.
 */
  ...
  for (let i = 0; i < retries; i++) {
    try {
      const buffer = await downloadMediaMessage(
        ...
        {
          logger: sock.logger,
          // IMPORTANT: request reupload via updateMediaMessage
...
/* ------------------------ post-broadcast reset/prompt ------------------- */

async function postBroadcastReset(username, sock, u, ownerBare) {
  // Clean up any pending state and timers
  ...
  // Prepare for next broadcast depending on mode
  if (u.mode === 'text') {
    u.awaitingPayload = 'text';
    await sendSys(username, sock, ownerBare, {
      text: `✍️ Ready. Type the *next message* to broadcast, or /stop to cancel.`
    });
  } else {
    u.awaitingPayload = null;
    await sendSys(username, sock, ownerBare, {
      text: `🖼️ Ready. Send the *next image* to broadcast. (/text to switch to text mode)`
    });
  }
}
...
async function handleBroadcastMessage(username, msg, sock) {
  ...
  if (!u.ownerJid) {
    // If this is a self chat, lock to the bare self JID; else lock to this chat
  ...
  if (selection && u.lastPromptChat === ownerBare && u.awaitingCategory && (u.pendingImage || u.pendingText)) {
    ...
    try {
      if (u.mode === 'text' && u.pendingText) {
        ...
        await sendSys(username, sock, ownerBare, { text: `🎉 All messages sent!` });
        await postBroadcastReset(username, sock, u, ownerBare);
        return;
      }
      ...
      // Safety net (main reset happens in postBroadcastReset on success)
      if (u.categoryTimeout) { clearTimeout(u.categoryTimeout); u.categoryTimeout = null; }
      u.awaitingCategory = false;
      u.broadcasting = false;
    }
