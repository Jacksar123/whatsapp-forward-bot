...
/* ---------------------------------------------------------------------- */
/*                          BUILD MARKER (for logs)                       */
/* ---------------------------------------------------------------------- */
console.log('[broadcast] build=warm406+r2 DIAG/targets/membership');

/* ---------------------------------------------------------------------- */
/*                              CONSTANTS                                 */
/* ---------------------------------------------------------------------- */

// Adaptive defaults (tuned safe). Pacing is per-user and auto-adjusts.
const DEFAULT_PACING = { batchSize: 2, parallel: 1, perSendDelay: 2000, batchDelay: 8000, lastAdjustAt: 0 };
const MIN_PACING     = { batchSize: 1, perSendDelay: 1200, batchDelay: 9000 };
const MAX_PACING     = { batchSize: 5, perSendDelay: 800,  batchDelay: 5000 };
...
function adjustPacing(u, stats) {
  ...
  if (rate >= 0.15 || (stats.timedOut || 0) > 0) {
    ...
    p.perSendDelay = Math.min(2400, Math.max(MIN_PACING.perSendDelay, p.perSendDelay + 120));
  } else if (rate === 0 && (stats.sentOk || 0) >= 5) {
    p.batchSize = Math.min(MAX_PACING.batchSize, p.batchSize + 1);
    p.batchDelay = Math.max(MAX_PACING.batchDelay, p.batchDelay - 400);
    p.perSendDelay = Math.max(MAX_PACING.perSendDelay, p.perSendDelay - 60);
  }
}
...
/* ---------------------------------------------------------------------- */
/*                     LIVE MEMBERSHIP / VISIBILITY                        */
/* ---------------------------------------------------------------------- */
async function buildLiveMembershipSet(sock) {
  try {
...
async function resolveTargets(username, sock, chosenCat){
  ...
  const memberSet = await buildLiveMembershipSet(sock);
...
async function sendToOneWithRetry(sock, jid, payload) {
  ...
  while (attempt <= SEND_MAX_RETRIES) {
    try {
      ...
      // Special: group sessions not ready yet → warm and continue same attempt
      if ((/No sessions/i.test(msg) || /not-acceptable/i.test(msg)) && jid?.endsWith?.('@g.us')) {
...
/* ---------------------------------------------------------------------- */
/*                 CORE: BATCH BROADCAST (ADAPTIVE/HEALING)                */
/* ---------------------------------------------------------------------- */
...
async function handleBroadcastMessage(username, msg, sock) {
  ...
  if (chatJid.endsWith('@g.us')) return; // ignore group chats
  if (!sock) return;
  ...
  if (body.toLowerCase().startsWith('/sendto ')) {
    ...
    // 1) BOT JID
    const botJid = sock?.user?.id || 'unknown@s.whatsapp.net';
    console.log(`[${username}] /sendto DIAG: BOT_JID=${botJid}`);

    // 2) group metadata & membership/admin
    try {
      const meta = await sock.groupMetadata(jid);
      const participants = (meta?.participants || []).map(p => ({ id: p.id, admin: p.admin }));
      const iAmIn = participants.some(p => p.id === botJid);
      const iAmAdmin = participants.some(p => p.id === botJid && (p.admin === 'admin' || p.admin === 'superadmin'));
      console.log(`[${username}] /sendto DIAG: group=${jid} subject="${meta?.subject}" announce=${!!meta?.announce} size=${participants.length} iAmIn=${iAmIn} iAmAdmin=${iAmAdmin}`);
    } catch (e) {
      console.warn(`[${username}] /sendto DIAG: groupMetadata(${jid}) error: ${e?.message || e}`);
    }
    ...
    // 4) warm+retry on 406/session errors
    const trySend = async (attempt) => {
      try {
        ...
        console.warn(`[${username}] /sendto attempt#${attempt+1} error: ${msg}`);
        if (/not-acceptable|No sessions|timed out|TIMEOUT:send/i.test(msg) && attempt < 3) {
          try { await warmSessionsForGroup(sock, jid); } catch {}
          const wait = 1200 * Math.pow(2, attempt) + Math.floor(Math.random()*300);
          await sleep(wait);
          return false;
        }
        throw e;
    ...
    try {
      let ok = false;
      for (let a=0; a<3 && !ok; a++) ok = await trySend(a);
      await sendSys(username, sock, ownerBare, { text: ok ? `✅ Sent to ${jid}` : `❌ Could not send to ${jid} after retries (not-acceptable)` });
    } catch (e) {
  ...
  if (body === '/help') {
    return await sendSys(username, sock, ownerBare, {
      text:
        ...
        `/sendto <jid> <text> - Send to one group directly (diagnostic)\n` +
        `/force on|off - Bypass membership preflight (debug)\n` +
