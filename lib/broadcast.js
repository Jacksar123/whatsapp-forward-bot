// lib/broadcast.js - Fixed WhatsApp broadcast system
const fs = require('fs');
...
async function handleBroadcastMessage(username, msg, sock) {
  ...
  // CRITICAL FIX: Enhanced debugging and simplified echo detection
  ...
  // CRITICAL FIX: Simplified echo detection - only ignore own non-command messages
  if (fromMe && !body.startsWith('/')) {
    console.log(`[msg] ${username}: Ignoring own non-command message: "${body.substring(0, 30)}"`);
    return;
  }
  ...
  // CRITICAL FIX: Better owner detection
  if (!u.ownerJid) {
    // Set owner from the first DM interaction
    ...
    u.mode = u.mode || 'media';
    console.log(`[msg] ${username}: OWNER SET TO: ${u.ownerJid} (from chat: ${chatJid})`);
  ...
  // CRITICAL FIX: Better owner matching
  const isFromOwner = (chatBare === ownerBare) || (chatJid === u.ownerJid);
  if (!isFromOwner) {
  ...
  // CRITICAL FIX: Better handling of unrecognized messages
