...
/** Anti-echo system send: record the message id we just sent and ignore it on rx */
async function sendSys(username, sock, jid, content, opts = {}) {
  const u = safeGetUser(username);
  if (!u.ignoreIds) u.ignoreIds = new Set();

  // prefer safeSend from index.js if present
  ...
  const res = await send(jid, content, opts).catch(e => {
    // swallow "Connection Closed" noise to avoid recursive loops
...
async function sendToGroupWithRetries({
  sock, jid, content, opts = {}, retryCount = DEFAULTS.retryCount
}) {
...
async function handleBroadcastMessage(username, msg, sock) {
  ...
  // 🔒 Anti-echo: if this id was sent by us, ignore it.
  try {
    const id = msg?.key?.id;
    if (id && u.ignoreIds && u.ignoreIds.has(id)) {
      u.ignoreIds.delete(id);
      return;
    }
  } catch {}
  ...
  try {
    ...
    const owner = u.ownerJid;
    const isOwner = !!owner && String(from) === String(owner);

    u.lastActive = Date.now();

    // Media from owner: your media handler will save u.lastMedia; do nothing here
    if (isOwner && msg.message.imageMessage && (u.mode === 'media' || !u.mode)) {
      log(username, 'Owner sent media — external media handler will persist buffer');
      return;
    }
    ...
    switch (cmd) {
      ...
      case '/sendto': {
        ...
        if (u.mode === 'text') {
          ...
          if (!body) {
            await sendSys(username, sock, u.ownerJid, { text: 'Please provide message text after the JID.' });
            break;
          }
      ...
      default: {
        ...
        if (u.lastPromptChat && u.lastPromptChat.type === 'choose_category') {
            ...
            if (num === cats.length + 1) {
              ...
              if (u.mode === 'media') {
                ...
                if (!u.pendingText) {
                  ...
                  await runBroadcast({ sock, username, jids: allJids, content: u.pendingText, contentType: 'text', force: u.force });
              ...
              if (u.mode === 'media') {
                ...
                if (!u.pendingText) {
                  ...
                  await runBroadcast({ sock, username, jids, content: u.pendingText, contentType: 'text', force: u.force });
                  await sendSys(username, sock, u.ownerJid, { text: '✅ Done. Send more text, or /media to switch.' });
                  delete u.pendingText;
                }
              }
          ...
          // Only save once per unique text (avoid spammy repeats)
          if (u.mode === 'text') {
            if (u.pendingText !== text) {
              u.pendingText = text;
