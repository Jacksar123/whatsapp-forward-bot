...
function bareJid(j) { return String(j || '').replace(/:[^@]+(?=@)/, ''); } // <-- critical fix: strip device suffix
...
async function autoScanAndCategorise(username, sock) {
  ...
  for (const g of groups) {
    ...
    allGroups[g.id] = { id: g.id, name, announce: !!g.announce };
  }
...
async function sendToOneWithRetry(sock, jid, payload) {
  ...
  while (attempt <= SEND_MAX_RETRIES) {
    try {
      ...
      // Special: group sessions not ready yet
      ...
      const retryable =
        /TIMEOUT:send|timed out|rate|temporar|retry|disconnect|socket|stream|closed|request_entity_too_large|too many|not-acceptable/i.test(msg);
...
async function handleBroadcastMessage(username, msg, sock) {
  ...
  // NEW: /mygroups — list live groups with real JIDs
  if (body === '/mygroups') {
    try {
      const metaMap = await sock.groupFetchAllParticipating();
      ...
      if (!rows.length) {
        await sendSys(username, sock, ownerBare, { text: 'You are not in any groups.' });
        return;
      }
      const CHUNK = 40;
      for (let i = 0; i < rows.length; i += CHUNK) {
        ...
        const text =
          '👥 *Your current WhatsApp groups (live)*\n' +
          slice.map(r => `- ${r.subject} — \`${r.jid}\`${r.announce ? ' (Announcement)' : ''} [${r.size}]`).join('\n');
        await sendSys(username, sock, ownerBare, { text });
        await sleep(400);
      }
    } catch (e) {
      await sendSys(username, sock, ownerBare, { text: `Failed to list groups: ${e.message}` });
    }
    return;
  }
  ...
  if (body.toLowerCase().startsWith('/sendto ')) {
    ...
    // DIAG: show bot identity & membership using bare JIDs (fix for :device suffix)
    try {
      const botJid = sock?.user?.id || '';
      console.log(`[${username}] /sendto DIAG: BOT_JID=${botJid}`);
      const meta = await sock.groupMetadata(jid).catch(() => null);
      const participants = (meta?.participants || []);
      const myBare = bareJid(botJid);
      const iAmIn = participants.some(p => bareJid(p.id) === myBare);
      const iAmAdmin = participants.some(p =>
        bareJid(p.id) === myBare && (p.admin === 'admin' || p.admin === 'superadmin')
      );
      console.log(
        `[${username}] /sendto DIAG: group=${jid} ` +
        `subject="${meta?.subject || meta?.name || jid}" announce=${!!meta?.announce} ` +
        `size=${participants.length} iAmIn=${iAmIn} iAmAdmin=${iAmAdmin}`
      );
    } catch (e) {
      console.warn(`[${username}] /sendto DIAG failed: ${e?.message || e}`);
    }
    ...
    try {
      ...
      console.warn(`[${username}] /sendto attempt#1 error: ${e?.message || e}`);
    }
    return;
  }
  ...
  if (body === '/help') {
    return await sendSys(username, sock, ownerBare, {
      text:
        ...
        `/mygroups - List current live groups with JIDs`
    });
  }
