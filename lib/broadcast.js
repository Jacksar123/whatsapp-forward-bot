...
// NEW: gentle human-like jitter and smarter backoff caps
const JITTER_MS = 400;
const BACKOFF_START_MS = 60_000;            // start at 1m
const BACKOFF_MAX_MS   = 10 * 60_000;       // ✅ cap at 10m (FIX for your crash)

/* --------------------------- adaptive pacing ---------------------------- */
/* Safer defaults + room to auto-tune up/down */
...
function adjustPacing(u, stats) {
  ...
  if (rate >= 0.15 || (stats.timedOut || 0) > 0) {
    ...
    p.perSendDelay = Math.min(2000, Math.max(MIN_PACING.perSendDelay, p.perSendDelay + 100)); // slower per-send
...
async function autoScanAndCategorise(username, sock) {
  ...
  // refresh allGroups from live data
  for (const g of groups) {
    const name = g.subject || g.name || g.id;
    allGroups[g.id] = { id: g.id, name };
  }

  // categorize new groups heuristically
  for (const g of groups) {
    const name = g.subject || g.name || g.id;
...
async function buildLiveMembershipSet(sock) {
  try {
    ...
    return new Set(); // fail-closed: no preflight
  }
}
 ...
 * - **NO hard aborts**; uses exponential backoff & continues
 * - respects /stop via cancel token
 * - membership preflight skips "not-member"
 */
async function sendInBatches(sock, username, from, jids, messageContent) {
  ...
  // NEW: backoff state that grows on throttling signals
  ...
  // live membership set for not-member preflight
  ...
  while (sent < total) {
    ...
    // tracking for honest status messages
    const actuallySent = [];
    ...
    for (const jid of batch) {
      ...
      attemptedNames.push(displayName);

      // preflight: skip if not in live membership set
      if (!memberSet.has(jid)) {
    ...
    if (sent >= EARLY_ABORT_CHECK_AT && stats.timedOut >= Math.ceil(EARLY_ABORT_CHECK_AT * 0.4)) {
      ...
      // after backoff, continue (no break)
    }
...
/**
 * Reconcile categories to live membership:
 * - Map names to current JIDs
 * - Drop JIDs bot isn’t a member of
 * - Persist to disk + supabase
 */
...
async function handleBroadcastMessage(username, msg, sock) {
  ...
  if (body === '/clean') {
    try {
      const msg2 = await cleanCategories(username, sock);
      return await sendSys(username, sock, ownerBare, { text: msg2 });
    } catch (e) {
      return await sendSys(username, sock, ownerBare, { text: `Clean failed: ${e.message}` });
    }
  }
