cp lib/broadcast.js lib/broadcast.backup.$(date +%s).js

cat > lib/broadcast.js <<'EOF'
const fs = require('fs');
...
async function autoScanAndCategorise(sock, username, USERS) {
  ...
  const existingGroups = readJSON(paths.groups, {});   // { jid: {id, name} }
  const categories = readJSON(paths.categories, {});   // { cat: [jid, ...] }

  for (const g of groups) {
    const name = g.subject || g.name || g.id;
    const jid = g.id;

    const isNew = !existingGroups[jid];
    existingGroups[jid] = { ...(existingGroups[jid] || {}), id: jid, name };

    // Only auto-append guesses for NEW groups (non-destructive)
    if (isNew) {
  ...
  // Persist atomically
  writeJSONAtomic(paths.groups, existingGroups);
  writeJSONAtomic(paths.categories, categories);

  USERS[username].allGroups = existingGroups;
  USERS[username].categories = categories;

  console.log(`[${username}] ✅ Auto-scan merged. Groups now: ${Object.keys(existingGroups).length}`);
}
...
async function handleBroadcastMessage(username, msg, USERS) {
  ...
  if (body === '/rescan' || body === '/syncgroups') {
    ...
    return await sock.sendMessage(from, { text: '✅ Rescanned and merged groups.' });
  }
  ...
  if (body === '/help') {
    return await sock.sendMessage(from, {
      text: `Commands:\n` +
            ...
            `/rescan or /syncgroups - Rescan groups (merge)\n` +
