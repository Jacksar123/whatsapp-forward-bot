...
// Quiet by default, mutexed broadcasts, 5-per-batch every 6s (env override)
...
// IMPORTANT: Option B (wait for category selection after text)
...
// Batch settings (ENV overridable)
const BATCH_SIZE        = parseInt(process.env.BATCH_SIZE || '5', 10);
const BATCH_INTERVAL_MS = parseInt(process.env.BATCH_INTERVAL_MS || '6000', 10);

// Status verbosity (defaults: QUIET ON, no per-batch posts)
// Set BATCH_STATUS_EVERY>0 to get a post every N batches when loud mode is on.
const BATCH_STATUS_EVERY = parseInt(process.env.BATCH_STATUS_EVERY || '0', 10); // 0 => only final summary
...
// Keyword map (only used if AUTO_MODE=true)
...
async function autoScanAndCategorise(username, sock) {
  ...
  try {
    ...
    // Auto-categorise only genuinely new groups
...
async function sendInBatches(sock, username, from, jids, messageContent, jobId) {
  ...
  // pre-warm sessions across all target groups
...
async function handleBroadcastMessage(username, msg, sock) {
  ...
  if (u.nudgeCooldownMs == null) u.nudgeCooldownMs = 30_000; // throttle nudges to 1 / 30s

  const chatJid = normaliseJid(msg.key.remoteJid);
  const fromMe = msg.key?.fromMe;
  const body = getMessageText(msg).trim();

  // ignore group chats entirely & status channel noise
  ...
  if (fromMe) {
    const sysPhrases = [
      ...
      // ignore our own nudge echoes to stop spam loops:
  ...
  if (body.startsWith('/addcategory ')) {
    ...
    // Refresh picker immediately if in text mode
    if (u.mode === 'text') {
  ...
  if (body.startsWith('/addgroup ')) {
    ...
    // If composing text (or picker is open), refresh the menu so it shows the new membership
  ...
  if (body === '/stop' || body === '/cancel') {
    requestCancel(u);
    u.broadcastJobId = null; // release mutex so running job sees it's not "mine"
    u.pendingText = null;
    u.pendingImage = null;
  ...
  if (body === '/text') {
    // stop any current job before switching
    requestCancel(u);
    ...
    // Show the category picker immediately in text mode
    const groupCount = Object.keys(u.allGroups || {}).length;
  ...
  // If user picks number while picker is open
  ...
  if (maybeNum && u.lastPromptChat === chatJid && u.awaitingCategory) {
    ...
    // If we already have content queued (rare in Option B), send now
    ...
    // No content yet → store target and prompt for text (still Option B friendly)
  ...
  if (u.mode === 'text' && body && !body.startsWith('/' ) && !u.awaitingCategory) {
    ...
    // If user preselected a target, honor it; otherwise show picker
    if (u.selectedTarget) {
    ...
    if (AUTO_MODE) {
      // Only used if you flip AUTO_MODE=true
    ...
    // Non-auto (Option B): prompt picker now
    const { text } = buildCategoryPrompt(username);
    u.awaitingCategory = true;
    u.lastPromptChat = chatJid;
    if (u.categoryTimeout) clearTimeout(u.categoryTimeout);
  ...
  // nudge (throttled + self-echo ignored above)
