// lib/broadcast.js - Complete automatic WhatsApp broadcast system
const fs = require('fs');
...
// Enhanced group scanning with membership verification
async function autoScanAndCategoriseWithCleanup(username, sock) {
  const u = USERSG()[username] || (USERSG()[username] = {});
  
  try {
    console.log(`[${username}] Starting enhanced group scan with cleanup...`);
    ...
    // Build verified groups list (only groups bot is actually in)
    const verifiedGroups = {};
    const removedGroups = [];
    let newGroups = 0;

    for (const g of groups) {
      // Verify bot is actually a participant
      const botInGroup = g.participants?.some(p => p.id === botJid);
      if (!botInGroup) {
        console.log(`[${username}] Skipping group "${g.subject}" - bot not a participant`);
        continue;
      }
      ...
      verifiedGroups[g.id] = { id: g.id, name };
    }

    // Clean categories - remove groups bot is no longer in
    const cleanedCategories = {};
    let removedFromCategories = 0;

    for (const [catName, jidList] of Object.entries(existingCats)) {
      if (!Array.isArray(jidList)) continue;
      
      const validJids = jidList.filter(jid => {
        if (verifiedGroups[jid]) {
          return true;
        } else {
          removedGroups.push({ jid, category: catName, groupName: u.allGroups?.[jid]?.name || jid });
      ...
      if (validJids.length > 0) {
        cleanedCategories[catName] = validJids;
      }
    }

    // Auto-categorize new groups
    for (const g of Object.values(verifiedGroups)) {
      const already = Object.values(cleanedCategories).some(list => list.includes(g.id));
      if (!already) {
        const guess = categoriseGroupName(g.name);
        if (guess) {
          (cleanedCategories[guess] ||= []).push(g.id);
          console.log(`[${username}] Auto-categorized "${g.name}" as ${guess}`);
        }
      }
    }

    // Update user data
    u.allGroups = verifiedGroups;
    u.categories = cleanedCategories;
    persistNow(username);

    // Log cleanup results
    console.log(`[${username}] ✅ Enhanced scan complete:`);
    console.log(`  - Verified groups: ${Object.keys(verifiedGroups).length}`);
    console.log(`  - New groups: ${newGroups}`);
    console.log(`  - Removed from categories: ${removedFromCategories}`);
    
    if (removedFromCategories > 0) {
      console.log(`[${username}] Removed groups:`, removedGroups.slice(0, 5));
    }

    return {
      verifiedGroups: Object.keys(verifiedGroups).length,
      newGroups,
      removedGroups: removedFromCategories,
      removedDetails: removedGroups
    };
    
  } catch (e) {
    console.error(`[${username}] Enhanced auto-scan failed: ${e.message}`);
    throw e;
  }
}

// Pre-broadcast membership verification
async function verifyGroupMembershipForBroadcast(sock, username, jids) {
  const u = USERSG()[username];
  const botJid = sock.user?.id;
  
  try {
    console.log(`[${username}] Verifying membership in ${jids.length} groups...`);
    
    // Get current participating groups
    const metaMap = await sock.groupFetchAllParticipating();
    const currentGroups = new Set();
    
    for (const [groupJid, groupInfo] of Object.entries(metaMap || {})) {
      const botInGroup = groupInfo.participants?.some(p => p.id === botJid);
      if (botInGroup) {
        currentGroups.add(groupJid);
      }
    }

    // Filter JIDs to only include groups bot is in
    const validJids = [];
    const invalidJids = [];
    
    for (const jid of jids) {
      if (jid.endsWith('@g.us') && !currentGroups.has(jid)) {
        invalidJids.push({
          jid,
          groupName: u.allGroups?.[jid]?.name || jid,
          reason: 'not_member'
        });
      } else {
        validJids.push(jid);
    ...
    console.log(`[${username}] Membership check: ${validJids.length} valid, ${invalidJids.length} invalid`);
    
    return { validJids, invalidJids, totalGroups: currentGroups.size };
    
  } catch (e) {
    console.warn(`[${username}] Membership verification failed: ${e.message}`);
    return { validJids: jids, invalidJids: [], totalGroups: 0 };
  }
}

// Automatic group cleanup on connection
async function autoCleanupOnConnection(username, sock) {
  try {
    console.log(`[${username}] Running automatic cleanup on connection...`);
    
    // Enhanced scan with cleanup
    const results = await autoScanAndCategoriseWithCleanup(username, sock);
    
    // Clean categories using the existing function
    const cleanResults = cleanCategories(username);
    
    console.log(`[${username}] Auto-cleanup complete:`, {
      ...results,
      cleanCategories: cleanResults
    });
    
    // Notify user if significant cleanup happened
    const u = USERSG()[username];
    if (u.ownerJid && (results.removedGroups > 3 || cleanResults.dropped > 3)) {
      try {
        await sendSys(username, sock, u.ownerJid, {
          text: `🧹 Automatic cleanup completed:\n` +
                `• Active groups: ${results.verifiedGroups}\n` +
                `• Removed stale groups: ${results.removedGroups + cleanResults.dropped}\n\n` +
                `Your bot is now synced with current group memberships.`
        });
      } catch {}
    }
    ...
    console.error(`[${username}] Auto-cleanup failed: ${e.message}`);
    return null;
  }
}
...
async function warmSessionsForGroup(sock, jid) {
  try {
    ...
    const botJid = sock.user?.id;
    const botInGroup = jids.includes(botJid);
    
    console.log(`[warm] Group ${jid}: ${jids.length} participants, bot in group: ${botInGroup}`);
    
    if (!botInGroup) {
      console.warn(`[warm] Bot not found in group ${jid} participants`);
      return 0;
    }
...
async function sendToOneWithRetry(sock, jid, payload) {
  let attempt = 0, warmed = false, checkedMembership = false;
  ...
  while (attempt <= SEND_MAX_RETRIES) {
    try {
      ...
      if (/not-acceptable|forbidden|unauthorized|No sessions/i.test(msg)) {
        ...
        // Check if bot is still in the group (only once per send attempt)
        if (!checkedMembership && jid.endsWith('@g.us')) {
          checkedMembership = true;
          try {
            console.log(`[send] Checking bot membership in group ${jid}`);
            const meta = await sock.groupMetadata(jid);
            const botJid = sock.user?.id;
            const botInGroup = meta.participants?.some(p => p.id === botJid);
            
            if (!botInGroup) {
              console.error(`[send] Bot is no longer a member of group ${jid}`);
              throw new Error(`BOT_NOT_IN_GROUP:${jid}`);
            }
            console.log(`[send] Bot confirmed in group ${jid} with ${meta.participants?.length} participants`);
          } catch (metaErr) {
            console.warn(`[send] Could not verify group membership for ${jid}: ${metaErr.message}`);
          }
        }
...
async function sendInBatchesWithAutoVerify(sock, username, from, jids, messageContent) {
  const u = USERSG()[username];
  const startTime = Date.now();

  console.log(`[batch] Starting smart broadcast to ${jids.length} groups for user ${username}`);
  ...
  const { validJids, invalidJids, totalGroups } = await verifyGroupMembershipForBroadcast(sock, username, jids);
  
  if (invalidJids.length > 0) {
    console.log(`[batch] Automatically filtered out ${invalidJids.length} invalid groups`);
    
    // Auto-cleanup categories if many groups are invalid
    if (invalidJids.length > 3) {
      console.log(`[batch] Many invalid groups detected, running auto-cleanup...`);
      await autoCleanupOnConnection(username, sock);
      
      // Notify user about cleanup
      try {
        await sendSys(username, sock, from, {
          text: `🧹 Cleaned up ${invalidJids.length} groups you're no longer in.\n` +
                `Broadcasting to ${validJids.length} valid groups...`
        });
      } catch {}
    }
  }
  ...
  if (validJids.length === 0) {
    await sendSys(username, sock, from, { 
      text: `❌ No valid groups to broadcast to. Bot may have been removed from groups.\n` +
            `Use /rescan to refresh your group list.`
    }).catch(()=>{});
    return;
  }

  // Proceed with broadcast using validated JIDs
  let sent = 0, failed = 0;
  const errors = [];
  ...
  for (let i = 0; i < validJids.length; i++) {
    ...
    if (isCancelled(u)) {
      console.log(`[batch] Broadcast cancelled by user at ${i}/${validJids.length}`);
      await sendSys(username, sock, from, { 
        text: `🛑 Broadcast cancelled. Sent: ${sent}/${validJids.length}` 
      }).catch(()=>{});
      break;
    }
    ...
    if (!u?.socketActive) {
      await sendSys(username, sock, from, { 
        text: `⚠️ Connection lost. Sent: ${sent}/${validJids.length}` 
      }).catch(()=>{});
      break;
    }

    try {
      console.log(`[batch] Sending to "${groupName}" (${i + 1}/${validJids.length})`);
      ...
      if (messageContent.text !== undefined) {
        ...
        try { 
          ...
          console.warn(`[batch] Image normalization failed: ${e.message}`);
        }
    ...
    // Rate limiting delay
    if (i < validJids.length - 1) {
      await sleep(SMALL_DELAY_MS);
    }
  }

  const totalTime = ((Date.now() - startTime) / 1000).toFixed(1);
  const successRate = validJids.length ? Math.round((sent/validJids.length) * 100) : 0;

  // Comprehensive report
  const report = [
    `📊 Broadcast Complete (${totalTime}s):`,
    `✅ Sent: ${sent}/${validJids.length}`,
    `📈 Success Rate: ${successRate}%`
  ];

  if (invalidJids.length > 0) {
    report.push(`🧹 Auto-filtered: ${invalidJids.length} stale groups`);
  }
...
async function handleBroadcastMessage(username, msg, sock) {
  ...
  if (body === '/rescan' || body === '/syncgroups') {
    ...
    await autoScanAndCategoriseWithCleanup(username, sock);
  ...
  if (maybeNum && u.lastPromptChat === ownerBare && u.awaitingCategory && (u.pendingImage || u.pendingText)) {
    ...
    if (u.mode === 'text' && u.pendingText) {
      ...
      await sendInBatchesWithAutoVerify(sock, username, ownerBare, jids, { text: u.pendingText });
      ...
      if (!imagePath || !fs.existsSync(imagePath)) {
        ...
        await sendInBatchesWithAutoVerify(
...
// Periodic group health check for all users
async function periodicGroupHealthCheck() {
  const users = Object.entries(USERSG());
  console.log(`[maintenance] Running periodic health check for ${users.length} users`);
  
  for (const [username, u] of users) {
    if (!u.socketActive || !u.sock) continue;
    
    try {
      const results = await autoCleanupOnConnection(username, u.sock);
      if (results && (results.removedGroups > 0)) {
        console.log(`[maintenance] Cleaned up ${results.removedGroups} stale groups for ${username}`);
      }
    } catch (e) {
      console.warn(`[maintenance] Health check failed for ${username}: ${e.message}`);
    }
    
    // Small delay between users to avoid overwhelming WhatsApp
    await sleep(2000);
  }
}
...
module.exports = {
  autoScanAndCategorise: autoScanAndCategoriseWithCleanup,
  buildCategoryPrompt,
  sendInBatches: sendInBatchesWithAutoVerify,
  handleBroadcastMessage,
  categoriseGroupName,
  cleanCategories,
  
  // New automatic management functions
  autoCleanupOnConnection,
  verifyGroupMembershipForBroadcast,
  periodicGroupHealthCheck
};