...
async function sendInBatches(sock, username, from, jids, messageContent, USERS) {
  ...
  while (sent < total) {
    ...
    sent += batch.length;
    if (sent < total) await sleep(BATCH_DELAY_MS); // 5 sec between each 5-group chunk
  }
}
...
async function handleBroadcastMessage(username, msg, USERS) {
  ...
  if (/^\/mode\s+(text|media)$/i.test(body)) {
    ...
    u.awaitingPayload = u.mode === 'text' ? 'text' : null; // ask for text next
    return await sock.sendMessage(from, {
      text:
        `✅ Mode changed. You are now in *${u.mode}* mode.\n` +
        (u.mode === 'text'
          ? `✍️ Reply with the message you want to broadcast.\n(After you send it, I'll ask where to send.)`
  ...
  if (body === '/text') {
    ...
    return await sock.sendMessage(from, {
      text: `✅ Mode changed. You are now in *text* mode.\n✍️ Reply with the message you want to broadcast.`
    });
  }
  if (body === '/media') {
    u.mode = 'media';
    ...
    return await sock.sendMessage(from, {
      text: `✅ Mode changed. You are now in *media* mode.\n🖼️ Send an image to start a broadcast.`
    });
  }
  ...
  if (body === '/cats') {
    // guard: in text mode, don't show categories until we have the text
    if (u.mode === 'text' && !u.pendingText) {
      return await sock.sendMessage(from, {
        text: `✍️ First, send the message you want to broadcast. Then I'll show categories.`
      });
    }
  ...
  // TEXT MODE: first require the user to provide the text payload
  if (u.mode === 'text') {
    ...
    // If not awaiting payload, and not awaiting category, but user sent a command or blank — do nothing
    // If they want to start over: they can /text again to re-arm awaitingPayload
  }
