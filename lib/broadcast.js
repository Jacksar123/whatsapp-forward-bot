...
// NEW: single place to cancel any active flow (text/media/category/broadcast)
...
async function handleBroadcastMessage(username, msg, sock) {
  ...
  // Ignore self system echoes, but allow commands ANYTIME (even during selection)
  if (fromMe) {
    if (!body) return;
    ...
    if (!body.startsWith('/')) {
      ...
      if (u.awaitingCategory) return; // still block random chatter during selection
    }
  }
