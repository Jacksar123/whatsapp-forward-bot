// lib/broadcast.js — AutoMode + Commands Fallback + Self-Chat Only + Per-Phone Isolation
// Date: 2025-09-22
...
async function autoScanAndCategorise(username, sock) {
  ...
  try {
    ...
    // rebuild allGroups strictly from live groups (prevents cross-phone leak)
    const allGroups = {};
    for (const g of groups) {
      allGroups[g.id] = { id: g.id, name: g.subject || g.name || g.id };
    }
    // hard-filter categories to only current device groups
    for (const k of Object.keys(existingCats)) {
      existingCats[k] = (existingCats[k] || []).filter(j => current.has(j));
      if (!existingCats[k].length) delete existingCats[k];
    }

    // auto-categorise only truly new groups
    const alreadyPlaced = new Set([].concat(...Object.values(existingCats)));
    for (const g of groups) {
      if (!alreadyPlaced.has(g.id)) {
        const guess = categoriseGroupName(g.subject || g.name || '');
...
async function handleBroadcastMessage(username, msg, sock) {
  ...
  if (fromMe) {
    ...
    // if we're awaiting a category and this is from us, ignore
  ...
  if (body === '/help') {
    const help = [
      '✅ *Commands*',
      '• /help — this help',
      '• /rescan — refresh groups & auto-categorise new ones',
      ...
      '',
      '*Auto Mode:* just send text or an image; I’ll infer the category and send. If unclear, I’ll ask.'
    ].join('\n');
  ...
  if (u.mode === 'text' && body && !body.startsWith('/' ) && !u.awaitingCategory) {
    ...
    if (AUTO_MODE) {
      ...
      // fallback: ALL if you want that behavior (comment out if you prefer number prompt)
