// lib/broadcast.js — Self-Chat Only, Robust Broadcast Engine (2025-09-22)
// Changes vs your last version:
// 1) ✅ STRICT SELF-CHAT GUARD: ignores all messages you send to others; only reacts in your "You" chat.
//    -> Added: `if (fromMe && chatBare !== selfBare) return;`
// 2) Everything else kept: batch sending w/ retries, auto-categorise, cleanup, timeouts, footer, etc.
...
// Batch settings: 5 groups every 10 seconds
const BATCH_SIZE = 5;             // 5 groups per batch
const SMALL_DELAY_MS = 10000;     // 10 seconds between batches
const SEND_MAX_RETRIES = 3;       // 3 retries per group
const SEND_TIMEOUT_MS = 45_000;   // 45 second timeout per send
...
async function autoScanAndCategorise(username, sock) {
  ...
  try {
    ...
    // Clean out categories of stale JIDs (optional)
    cleanCategories(username);
...
function cleanCategories(username) {
  ...
  console.log(`[${username}] cleanCategories: kept=${kept} fixed=${fixed} dropped=${dropped}`);
  persistNow(username);
...
async function sendInBatches(sock, username, from, jids, messageContent) {
  ...
  console.log(`[batch] Starting broadcast to ${jids.length} groups (batches of ${BATCH_SIZE})`);
  ...
  for (let batchStart = 0; batchStart < jids.length; batchStart += BATCH_SIZE) {
    ...
    if (isCancelled(u)) {
      console.log(`[batch] Broadcast cancelled by user at batch ${batchNum}/${totalBatches}`);
    ...
    console.log(`[batch] Processing batch ${batchNum}/${totalBatches} (${batch.length} groups)`);

    // Send to all groups in this batch simultaneously
      ...
      try {
        if (messageContent.text !== undefined) {
          ...
          if (!fs.existsSync(messageContent.image.url)) {
            ...
            return { success: false, jid, groupName, error: 'File not found', skipped: true };
          }
        ...
        console.log(`[batch] ✅ Sent to "${groupName}"`);
        return { success: true, jid, groupName };

      } catch (e) {
        const errorMsg = String(e?.message || e);
        console.warn(`[batch] ❌ Failed "${groupName}": ${errorMsg}`);
        return { success: false, jid, groupName, error: errorMsg };
      }
    });
    ...
    await sendSys(username, sock, from, {
      text: `Batch ${batchNum}/${totalBatches} complete (${elapsed}s)\n✅ Sent: ${sent} | ❌ Failed: ${failed} | ⭐ Skipped: ${skipped}`
    }).catch(()=>{});

    // Check if any fatal errors occurred (socket issues)
    const hasFatalError = errors.some(err =>
      /SOCKET_NOT_OPEN|Connection Closed|stream closed/i.test(err.error)
    );

    if (hasFatalError) {
      console.error(`[batch] Fatal error detected, stopping broadcast`);
      break;
    }

    // Wait 10 seconds before next batch (except for last batch)
    if (batchStart + BATCH_SIZE < jids.length) {
      console.log(`[batch] Waiting ${SMALL_DELAY_MS / 1000} seconds before next batch...`);
...
async function handleBroadcastMessage(username, msg, sock) {
  ...
  // Enhanced debugging for self-messaging
  ...
  // ✅ STRICT SELF-CHAT GUARD:
  // Only process messages in your own "You" chat. Any message you send to other contacts is ignored.
  if (fromMe && chatBare !== selfBare) return;

  // For self-messaging bots, owner is the bot's own number
  if (!u.ownerJid) {
  ...
  // For self-messaging, ignore obvious system echoes
  if (fromMe) {
  ...
  if (body === '/test') {
    ...
    return await sendSys(username, sock, u.ownerJid, {
      text: `🧪 Bot Status:\n` +
            ...
            `• Self-messaging: ✅\n` +
            `• Batch size: ${BATCH_SIZE} groups every ${SMALL_DELAY_MS/1000}s\n\n` +
  ...
  /* ----------- CRITICAL: Check numeric category selection FIRST ----------- */
  ...
  if (maybeNum && u.lastPromptChat === u.ownerJid && u.awaitingCategory && (u.pendingImage || u.pendingText)) {
    ...
    if (u.mode === 'text' && u.pendingText) {
      await sendSys(username, sock, u.ownerJid, { text: `Broadcasting *text* to ${jids.length} group(s) in batches of ${BATCH_SIZE}...` });
      ...
      if (!imagePath || !fs.existsSync(imagePath)) {
        ...
        await sendSys(username, sock, u.ownerJid, { text: `Broadcasting *image* to ${jids.length} group(s) in batches of ${BATCH_SIZE}...` });
        await sendInBatches(
  ...
  // Better handling of unrecognized messages in self-chat
