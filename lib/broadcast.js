...
async function handleBroadcastMessage(username, msg, USERS) {
  ...
  if (u.mode === 'text') {
    ...
    // If user sends random stuff but we're NOT awaiting payload, ignore
  }
  ...
  if (m.imageMessage && u.mode === 'media') {
    ...
    const imagePath = path.join(getUserPaths(username).tmp, `image_${theTimestamp}.jpg`);
