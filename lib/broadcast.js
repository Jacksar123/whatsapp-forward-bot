...
const { normaliseImage } = require('./media-normalise'); // ✅ ensure valid image+mime

/* ----------------------------- constants -------------------------------- */

// Sending pace (tuned for stability)
const BATCH_SIZE = 5;                 // ✅ slightly larger batches, but…
const PARALLEL_PER_SLICE = 1;         // ✅ …we serialize sends to avoid rate-limit stalls
const PER_SEND_DELAY_MS = 250;        // small delay between sends inside a slice
const BATCH_DELAY_MS = 4000;          // delay between batches
...
async function sendInBatches(sock, username, from, jids, messageContent) {
  ...
  // ✅ Normalize image & set mimetype to avoid "blank/invalid media"
  if (messageContent.image?.url) {
    ...
    try {
      ...
      imageBuffer = safeBuf;
      imageMime = mimetype;
      console.log(`[${username}] media validated size=${safeBuf.length}B type=${mimetype}`);
    } catch (e) {
      console.warn(`[${username}] media normalisation failed: ${e.message}`);
      imageBuffer = raw; // fallback to raw
    }
  }
  ...
  while (sent < total) {
    ...
    // small slices with limited parallelism — serialize to avoid WL stalls
    let idx = 0;
    ...
    await sendSys(username, sock, from, {
      text: `✅ Sent to:\n${groupNames.map(n => `- ${n}`).join('\n')}\n\n${
        sent + batch.length < total ? `⏳ Next batch in ${Math.round(BATCH_DELAY_MS/1000)}s…` : '🎉 All messages sent!'
      }`,
    });
...
async function handleBroadcastMessage(username, msg, sock) {
  ...
  if (u.mode === 'media' && hasImage(msg)) {
    ...
    ensureDir(up.tmp);
    const imagePath = path.join(up.tmp, `image_${timestamp}.bin`); // keep extension-agnostic; we normalise at send
