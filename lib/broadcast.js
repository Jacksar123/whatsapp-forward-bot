...
const {
  ...
  prepareWAMessageMedia, // ✅ function export from Baileys
...
// ✅ Pre-upload image once and reuse for the whole broadcast
async function prepareReusableImage(sock, imagePathOrBuffer) {
  ...
  return prepared.image; // <- reuse this for all sends
}

// ✅ 5-per-5s, but each batch sends its 5 **in parallel** (so a batch doesn't take 25s)
async function sendInBatches(sock, username, from, jids, messageContent, USERS) {
  ...
  // If image, pre-upload once
  let waImage = null;
  let baseCaption = messageContent.caption || '';
  if (messageContent.image?.url) {
    try {
      waImage = await prepareReusableImage(sock, messageContent.image.url);
    } catch (e) {
    ...
    await Promise.all(batch.map(async (jid) => {
      try {
        if (messageContent.text !== undefined) {
          ...
          await safeSend(jid, { image: waImage, caption, contextInfo: { forwardingScore: 2, isForwarded: true } });
