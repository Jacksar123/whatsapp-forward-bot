...
async function handleBroadcastMessage(username, msg, sock) {
  ...
  // 🔒 HARD GATE: only process messages you send to your own self-chat.
  if (!(fromMe && chatBare === selfBare)) return;

  // (groups/status would be filtered by the gate anyway)
  ensureOwnerConsistency(username, selfBare);

  // Ignore our own system echoes BUT allow slash-commands even when picker is open
  if (fromMe) {
    ...
    if (u.awaitingCategory && !isCommand) {
      // let numeric handler below consume it
    }
  }
