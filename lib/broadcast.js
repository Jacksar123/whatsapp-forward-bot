...
/* ---------- FIX: unwrap helpers for ephemeral / viewOnce wrappers -------- */
...
function getContent(msg) {
  // Always return the deepest content level
  let c = msg?.message || {};
  let prev = null;
...
async function handleBroadcastMessage(username, msg, sock) {
  ...
  // 4) Owner‑DM ONLY — learn owner from first inbound DM and greet once
  if (!u.ownerJid) {
    u.ownerJid = chatJid;
    u.mode = u.mode || 'media';
    if (!u.greeted) {
      u.greeted = true;
      await sendSys(username, sock, chatJid, {
        text:
          `✅ Connected.\n\n` +
          `Use:\n` +
  ...
  // 5) Anti-echo: ignore our own system messages
  ...
  if (u.mode === 'media' && hasImage(msg)) {
    // Download using the original msg (Baileys needs the full wrapper to decrypt)
