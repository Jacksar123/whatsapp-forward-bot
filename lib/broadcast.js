...
// Pace: 5 groups per 5 seconds (as requested)
const BATCH_SIZE = 5;                 // groups per batch
const PARALLEL_PER_SLICE = 2;         // tiny concurrency inside a batch
const PER_SEND_DELAY_MS = 250;        // delay between individual sends
const BATCH_DELAY_MS = 5000;          // wait between batches

// Robustness
const SEND_MAX_RETRIES = 3;           // retry per group
...
/**
 * Robust sender with retries & backoff to survive rate limits / session churn.
 */
async function sendToOneWithRetry(sock, jid, payload) {
  ...
  while (attempt <= SEND_MAX_RETRIES) {
    try {
      ...
      const retryable =
        /timed out|rate|temporar|retry|disconnect|socket|stream|closed|request_entity_too_large|too many/i.test(msg);
      attempt++;
...
async function handleBroadcastMessage(username, msg, sock) {
  ...
  if (selection && u.lastPromptChat === ownerBare && u.awaitingCategory && (u.pendingImage || u.pendingText)) {
    ...
    try {
      if (u.mode === 'text' && u.pendingText) {
        ...
        // Always nudge after finish
        await sendSys(username, sock, ownerBare, { text: `✍️ Done. Send another message to broadcast, or /stop to cancel.` });
        setTimeout(() => {
          sendSys(username, sock, ownerBare, { text: `Ready for the next one. Type your message or use /cats.` }).catch(()=>{});
        }, 1200);
      ...
      if (u.pendingImage) {
        ...
        if (u.mode === 'text') {
          ...
          setTimeout(() => {
            sendSys(username, sock, ownerBare, { text: `Ready for the next one. Send an image now, or /text to switch.` }).catch(()=>{});
          }, 1200);
          u.awaitingPayload = null;
        }
  ...
  if (u.mode === 'media' && hasImage(msg)) {
    // Use retrying media download to avoid "Waiting for this message..."
