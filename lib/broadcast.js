// lib/broadcast.js - Complete replacement with improved error handling and debugging
const fs = require('fs');
...
async function sendInBatches(sock, username, from, jids, messageContent) {
  ...
  if (!messageContent.text && !messageContent.image?.url) {
    console.error(`[batch] Invalid message content - no text or image`);
  ...
  for (let i = 0; i < jids.length; i++) {
    ...
    try {
      ...
      // Stop if too many BOT_NOT_IN_GROUP errors
      if (errorMsg.includes('BOT_NOT_IN_GROUP') && failed >= 5) {
        console.error(`[batch] Too many membership errors, bot may have been removed from groups`);
        break;
      }
    }
