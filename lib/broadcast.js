...
theBACKOFF_MAX_MS   = 10 * 60_000;       // cap at 10m
...
async function canWriteToGroup(sock, jid) {
  try {
    const self = getSelfId(sock);
    const meta = await sock.groupMetadata(jid);
    const me = (meta?.participants || []).find(p => p?.id === self);
    if (!me) return { ok: false, reason: 'not-member' };

    const announce = String(meta?.announce || '').toLowerCase() === 'true';
    const commAnn  = String(meta?.isCommunityAnnounce || '').toLowerCase() === 'true';
    const admin    = me.admin === 'admin' || me.admin === 'superadmin';

    if ((announce || commAnn) && !admin) return { ok: false, reason: 'announce-nonadmin' };
    return { ok: true };
  } catch {
    return { ok: false, reason: 'meta-error' };
  }
}

/* --------------------- refresh stale name→jid map ----------------------- */

async function refreshGroupIdsFromNames(username, sock) {
  const u = (global.USERS[username] ||= {});
  const metaMap = await sock.groupFetchAllParticipating();
  const byName = new Map(Object.values(metaMap || {}).map(g => [ (g.subject || g.name || '').toLowerCase(), g.id ]));

  let changes = 0;
  for (const [cat, list] of Object.entries(u.categories || {})) {
    const newList = [];
    for (const item of list) {
      if (item && item.endsWith('@g.us')) { newList.push(normaliseGroupJid(item)); continue; }
      const name = String(item || '').toLowerCase();
      const jid  = byName.get(name);
      if (jid) { newList.push(normaliseGroupJid(jid)); changes++; }
    }
    u.categories[cat] = Array.from(new Set(newList.filter(Boolean)));
  }
  if (changes) persistNow(username);
  return changes;
}
  ...
  while (attempt <= SEND_MAX_RETRIES) {
    try {
      ...
      if (/not-acceptable/i.test(msg)) {
        // Permission/room-type issue — don’t waste retries
        throw err;
      }
 ...
 * - refreshes stale name entries to live JIDs
 * - normalises image once
 * - preflights write permission per group
...
async function sendInBatches(sock, username, from, jids, messageContent) {
  ...
  // Refresh stale name→jid mappings once per run
  try { await refreshGroupIdsFromNames(username, sock); } catch {}
  ...
  while (sent < total) {
    ...
    for (const jid of batch) {
      ...
      // preflight: skip unwritable groups to avoid "not-acceptable"
      const gate = await canWriteToGroup(sock, jid);
      if (!gate.ok) {
        console.warn(`[${username}] ⛔️ Skipping ${jid} (${gate.reason})`);
        stats.failed++;
...
async function diagGroup(sock, jid) {
  const selfId = getSelfId(sock);
  const meta = await sock.groupMetadata(jid);
  const me = (meta?.participants || []).find(p => p?.id === selfId);
  return {
    jid,
    name: meta?.subject || meta?.name || jid,
    announce: String(meta?.announce || '').toLowerCase(),
    isCommunityAnnounce: String(meta?.isCommunityAnnounce || '').toLowerCase(),
    size: (meta?.participants || []).length,
    amInGroup: !!me,
    myRole: me?.admin || 'member',
    parentCommunityJid: meta?.parentJid || null,
  };
}
...
async function handleBroadcastMessage(username, msg, sock) {
  ...
  if (body === '/rescan' || body === '/syncgroups') {
    ...
    // also refresh names→JIDs in cats
    try { await refreshGroupIdsFromNames(username, sock); } catch {}
  ...
  if (body.startsWith('/diag ')) {
    ...
    const jid = target.endsWith('@g.us')
      ? target
      : Object.keys(u.allGroups || {}).find(j => (u.allGroups[j]?.name || '').toLowerCase() === target.toLowerCase());

    if (!jid) return await sendSys(username, sock, ownerBare, { text: `Group not found: ${target}` });

    try {
      const d = await diagGroup(sock, jid);
      await sendSys(username, sock, ownerBare, {
        text:
          `🧪 DIAG\n` +
          `Name: ${d.name}\nJID: ${d.jid}\nInGroup: ${d.amInGroup}\nMyRole: ${d.myRole}\n` +
          `Announce: ${d.announce}\nCommunityAnnounce: ${d.isCommunityAnnounce}\n` +
          `ParentCommunity: ${d.parentCommunityJid || 'none'}\nMembers: ${d.size}`
      });
    } catch (e) {
      await sendSys(username, sock, ownerBare, { text: `Diag failed: ${e.message}` });
    }
    return;
  }
  ...
  if (body === '/help') {
    return await sendSys(username, sock, ownerBare, {
      text:
        ...
        `/diag <jid|name> - Inspect a group’s permissions & flags\n` +
