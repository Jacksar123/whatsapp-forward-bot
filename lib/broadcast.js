...
async function sendSys(USERS, username, sock, jid, content) {
  ...
  try {
    ...
    if (id) {
      u.ignoreIds.add(id);
      // Trim set if it grows too large (keep last ~500)
  ...
  while (sent < total) {
    ...
    for (const jid of batch) {
      try {
        ...
        // 🔥 Always inject footer before sending
        if (messageContent.text !== undefined) {
          messageContent.text = withFooter(messageContent.text || '');
        }
        if (messageContent.caption !== undefined) {
          messageContent.caption = withFooter(messageContent.caption || '');
        }

        await send(jid, messageContent);
  ...
  if (selection && u.lastPromptChat === from && u.awaitingCategory && (u.pendingImage || u.pendingText)) {
    ...
    if (u.pendingImage) {
      ...
      await sendInBatches(
        ...
        jids,
        {
          image: { url: imagePath },       // ✅ file-path send avoids prepareWAMessageMedia
          mimetype: 'image/jpeg',
