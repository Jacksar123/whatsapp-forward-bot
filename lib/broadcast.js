...
// Batch pacing: 5 recipients per batch, 6s between batches (configurable via env)
// Date: 2025-09-23
...
async function handleBroadcastMessage(username, msg, sock) {
  ...
  if (body === '/help') {
    const help = [
      ...
      '',
      `*Batching:* ${BATCH_SIZE} groups per batch, ${BATCH_INTERVAL_MS}ms between batches (set BATCH_SIZE / BATCH_INTERVAL_MS env).`,
