...
// Strict batch pacing: 5 groups every 5 seconds
const BATCH_SIZE = 5;                 // exactly 5 per batch
const BATCH_DELAY_MS = 5000;          // 5s between batches
const PER_SEND_DELAY_MS = 0;          // no extra delay inside a batch
...
/**
 * Retry + backoff for each individual send.
 */
  ...
  while (sent < total) {
    ...
    for (const jid of batch) {
      try {
        ...
        if (PER_SEND_DELAY_MS) await sleep(PER_SEND_DELAY_MS);
      } catch (error) {
        console.error(`[${username}] ❌ Failed to send to ${jid}:`, error?.message || error);
      }
    }

    // progress ping to the owner chat
    ...
    // wait exactly 5 seconds between batches (unless finished)
