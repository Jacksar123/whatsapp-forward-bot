...
// pacing (adaptive loop adjusts these while running)
...
const BACKOFF_START_MS   = 60_000;          // throttle backoff start 1m
const BACKOFF_MAX_MS     = 10 * 60_000;     // cap at 10m
const JITTER_MS          = 400;

const INTERACTION_TIMEOUT_MS = 30 * 60 * 1000; // 30 mins
...
async function ensureSessionForJid(sock, jid) {
  try {
    // For groups: touching metadata helps Baileys initialise sender keys
    if (jid.endsWith('@g.us') && typeof sock.groupMetadata === 'function') {
      await withTimeout(sock.groupMetadata(jid), 5_000, `meta:${jid}`);
    }
  } catch (e) {
    // metadata can fail if you’re not a member — ignore here
  }
  try {
    if (typeof sock.assertSessions === 'function') {
      // false = do not force refresh every time; quicker.
      await withTimeout(sock.assertSessions([jid], false), 8_000, `assert:${jid}`);
    }
  } catch (e) {
    // We'll still try to send — if it still says No sessions, we’ll requeue once
  }
  await sleep(300); // tiny settle delay
}

/* --------------------------- media download ----------------------------- */

async function safeDownloadMedia(msg, sock, retries = 10, delay = 3000) {
  await sleep(800); // allow WA to finish upload + keys
...
async function sendToOneWithRetry(sock, jid, payload) {
  ...
  while (attempt <= SEND_MAX_RETRIES) {
    try {
      ...
      // Hard socket failure — surface and let outer layer decide
      ...
      if (isNoSessions) {
        // Re-preflight once and short backoff — then try again
        try { await ensureSessionForJid(sock, jid); } catch {}
      }

      const base = isNoSessions ? 1200 : 1200;
...
async function sendInBatches(sock, username, from, jids, messageContent) {
  ...
  while (nextIndex < pendingQueue.length) {
    ...
    for (const jid of batch) {
      ...
      try {
        // preflight **per JID** (critical for “No sessions”)
        await ensureSessionForJid(sock, jid);
        ...
        // If “No sessions”, requeue exactly once after a deliberate preflight
        if (isNoSessions && !requeuedOnce.has(jid)) {
          requeuedOnce.add(jid);
          try { await ensureSessionForJid(sock, jid); } catch {}
          pendingQueue.push(jid);
    ...
    // Throttle signals → backoff & continue (no aborts)
