...
async function handleBroadcastMessage(username, msg, USERS) {
  ...
  if (selection && u.pendingImage && u.lastPromptChat === from) {
    ...
    let rawList = chosen === '__ALL__' ? Object.keys(groups) : (cats[chosen] || []);

    // ✅ Auto-convert group names to JIDs if needed
    const jids = rawList.map(entry => {
      if (entry.endsWith('@g.us')) return entry;
      const match = Object.values(groups).find(g => g.name === entry);
      if (match && match.id && match.id.endsWith('@g.us')) return match.id;
      console.warn(`[${username}] ⚠️ Could not resolve group "${entry}" to a valid JID.`);
      return null;
    }).filter(Boolean);
