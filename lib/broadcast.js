...
// Baseline pacing (adaptive loop will adjust further)
const SEND_MAX_RETRIES   = 5;          // retries per target on failure
const SEND_BACKOFF_MS    = 1500;       // base backoff for exponential retry
const SEND_TIMEOUT_MS    = 35000;      // hard timeout for a single send
...
const JITTER_MS        = 500;
const BACKOFF_START_MS = 60_000;       // 1m on detected throttling
const BACKOFF_MAX_MS   = 15 * 60_000;  // cap at 15m

/* --------------------------- adaptive pacing ---------------------------- */

const DEFAULT_PACING = { batchSize: 3, parallel: 1, perSendDelay: 400, batchDelay: 7000, lastAdjustAt: 0 };
const MIN_PACING     = { batchSize: 2, perSendDelay: 300, batchDelay: 8000 };
...
function looksLikeThrottle(errMsg) {
  ...
  return (
    ...
    s.includes('timeout') ||
    s.includes('socket') ||
    s.includes('stream closed')
  );
}

function isMeAdminIn(meta, meBare) {
  const me = (meta?.participants || []).find(p => bareJid(p?.id) === meBare);
  return !!(me && (me.admin === 'admin' || me.admin === 'superadmin' || me.isAdmin));
}

// If announce=true and we’re not admin -> clean skip (don’t thrash)
async function canSendToGroup(sock, gid, meBare) {
  try {
    const meta = await withTimeout(sock.groupMetadata(gid), 8000, `meta-check:${gid}`);
    if (!meta?.announce) return { ok: true, reason: null };
    return isMeAdminIn(meta, meBare) ? { ok: true, reason: null } : { ok: false, reason: 'admins_only' };
  } catch {
    return { ok: false, reason: 'meta_unavailable' };
  }
}

// Assert Signal sessions for *participants* (NOT the group JID)
async function assertParticipantsSessions(sock, gid) {
  try {
    const meta = await withTimeout(sock.groupMetadata(gid), 8000, `meta:${gid}`);
    ...
    if (typeof sock.assertSessions === 'function' && parts.length) {
      ...
      for (let i = 0; i < parts.length; i += CHUNK) {
        ...
        try { await withTimeout(sock.assertSessions(chunk, false), 15000, `assertParts:${gid}:${i}`); } catch {}
        await sleep(120);
      }
    }
  } catch {}
...
async function autoScanAndCategorise(username, sock) {
  ...
  const meBare = bareJid(sock?.user?.id || '');

  for (const g of groups) {
    const name = g.subject || g.name || g.id;
    // capture announce/admin if available to help UI
    const announce = !!g.announce;
    let botIsAdmin = false;
    try {
      if (g?.participants?.length) {
        const me = g.participants.find(p => bareJid(p?.id) === meBare);
        botIsAdmin = !!(me && (me.admin === 'admin' || me.admin === 'superadmin' || me.isAdmin));
      }
    } catch {}
    allGroups[g.id] = { id: g.id, name, announce, botIsAdmin };
 ...
 */
async function safeDownloadMedia(msg, sock, retries = 10, delay = 3500) {
...
/** Single target sender with permission gate + session assert + retries */
async function sendToOneWithRetry(sock, jid, payload) {
  ...
  while (attempt <= SEND_MAX_RETRIES) {
    try {
      // One-time preflight per JID: permission gate + assert sessions
      if (!preflightDone) {
        if (jid.endsWith('@g.us')) {
          const perm = await canSendToGroup(sock, jid, meBare);
          if (!perm.ok) {
            // clean skip: do not keep retrying admins-only/meta-unavailable
            throw new Error(`SKIP:${perm.reason}`);
          }
          await assertParticipantsSessions(sock, jid);
        } else if (typeof sock.assertSessions === 'function') {
          await withTimeout(sock.assertSessions([bareJid(jid)], false), 9000, `assert:${jid}`);
        }
        preflightDone = true;
      }
 ...
 * - pre-warms participant sessions for all targets
 * - adapts pacing up/down per user
 * - skips admins-only + repeated failures
 * - requeues "No sessions" via pre-assert on retry path
 * - early throttle detection + backoff softening
...
async function sendInBatches(sock, username, from, jids, messageContent) {
  ...
  const badJids = new Map();  // jid -> fail count (excludes clean 'SKIP:' cases)
  ...
  // PREWARM: assert participant sessions once per target group
  try {
    await sendSys(username, sock, from, { text: `🔧 Preparing sessions for ${pendingQueue.length} group(s)…` });
    for (const gid of pendingQueue) {
      if (String(gid).endsWith('@g.us')) { await assertParticipantsSessions(sock, gid); }
      await sleep(120);
    }
  ...
  while (nextIndex < pendingQueue.length) {
    ...
    for (const jid of batch) {
      ...
      try {
        ...
        if (msg.startsWith('SKIP:')) {
          skippedThisBatch.push(jid);
          lastProgressAt = Date.now();
          console.warn(`[${username}] ⏭️  Skipped ${jid} (${msg.slice(5)})`);
          continue;
        }
    ...
    const namesSkip = skippedThisBatch.map(j => USERS[username].allGroups[j]?.name || j);

    let statusText = '';
    if (namesOk.length)   statusText += `✅ Sent to:\n${namesOk.map(n => `- ${n}`).join('\n')}\n\n`;
    if (namesSkip.length) statusText += `⏭️ Skipped (admins-only/meta):\n${namesSkip.map(n => `- ${n}`).join('\n')}\n\n`;
    ...
    if ((namesOk.length + namesFail.length + namesSkip.length) >= Math.min(batch.length, EARLY_ABORT_CHECK_AT) &&
  ...
  const skippedFails = Array.from(badJids.entries()).filter(([, n]) => n >= 2).map(([jid]) => jid);
  if (skippedFails.length) {
    const names = skippedFails.map(j => USERS[username].allGroups[j]?.name || j).slice(0, 20);
    await sendSys(username, sock, from, {
      text: `ℹ️ Skipped ${skippedFails.length} group(s) after repeated failures:\n` +
            names.map(n => `- ${n}`).join('\n') + (skippedFails.length > names.length ? `\n...` : '')
    });
  }
...
async function handleBroadcastMessage(username, msg, sock) {
  ...
  if (chatJid.endsWith('@g.us')) return; // ignore groups for control flow
  if (!sock) return;
  ...
  if (u.mode === 'media' && hasImage(msg)) {
    ...
    const buffer = await safeDownloadMedia(msg, sock, 10, 3500);
