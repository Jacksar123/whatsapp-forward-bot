 ...
 * Addresses "No sessions" / `not-acceptable` right after relink.
 * Falls back to poking the keystore when `sock.assertSessions` isn't present.
 */
async function warmSessionsForGroup(sock, jid) {
  try {
    ...
    // Fallback: prod authState key stores so prekeys/sender-keys load
...
async function sendToGroupWithRetries({ sock, jid, content, opts = {}, retryCount = DEFAULTS.retryCount }) {
  ...
  for (let attempt = 0; attempt <= retryCount; attempt++) {
    try {
      ...
      if (/not-acceptable/i.test(msg) && didWarmup) {
        return { ok: false, permanent: true, error: msg };
      }
  ...
  try {
    for (let i = 0; i < jids.length; i += batchSize) {
      ...
      for (const jid of batch) {
        ...
        // PRE-SKIP: announcement-only groups if we're not admin
        if (meta.announce && !meta.iAmAdmin) {
          results.skipped++; batchFails++;
          results.details.push({ jid, ok: false, reason: 'announce-group-nonadmin' });
          await sleep(150);
          continue;
        }
  ...
  try {
    ...
    switch (cmd) {
      ...
      case '/syncgroups':
        ...
        // auto-clean after scan to fix stale JIDs
        try {
      ...
      case '/sendto': {
        ...
        // PRE-SKIP if announce & not admin
        if (meta.announce && !meta.iAmAdmin) {
          await sendSys(username, sock, u.ownerJid, { text: `⚠️ ${meta.subject} is announcement-only and you're not admin — skipping.` });
          break;
        }
        ...
        if (u.lastPromptChat && u.lastPromptChat.type === 'choose_category') {
          ...
          if (!isNaN(num) && num >= 1 && num <= cats.length + 1) {
            ...
            if (u.mode === 'media') {
              ...
              if (!u.pendingText) {
                await sendSys(username, sock, u.ownerJid, { text: 'No pending text to broadcast. Send a message first or /media to switch.' });
              } else {
          ...
          // Friendlier text capture: if owner sends a plain line, save it & (optionally) switch to text
