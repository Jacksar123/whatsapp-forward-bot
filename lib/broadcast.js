...
// Conservative baseline. We'll adapt up if clean.
const BATCH_SIZE = 2;                 // ↓ from 3
const PARALLEL_PER_SLICE = 1;         // keep serialized (Baileys likes this)
const PER_SEND_DELAY_MS = 900;        // ↑ from 250 (human-ish cadence)
const BATCH_DELAY_MS = 9000;          // ↑ from 6000
const SEND_MAX_RETRIES = 5;
const SEND_BACKOFF_MS = 1500;         // ↑ a touch
const SEND_TIMEOUT_MS = 35000;        // ↑ from 20000 (mobile radios can be slow)
...
const JITTER_MS = 600;                // ↑ small jitter
const BACKOFF_START_MS = 60_000;      // 1m on throttle
const BACKOFF_MAX_MS   = 15 * 60_000; // cap at 15m

/* --------------------------- adaptive pacing ---------------------------- */

const DEFAULT_PACING = { batchSize: 2, parallel: 1, perSendDelay: 900, batchDelay: 9000, lastAdjustAt: 0 };
const MIN_PACING     = { batchSize: 1, perSendDelay: 800, batchDelay: 9000 };
const MAX_PACING     = { batchSize: 4, perSendDelay: 500, batchDelay: 5000 };
...
function adjustPacing(u, stats) {
  ...
  // If anything goes wrong, slow down gently.
  if (rate >= 0.10 || (stats.timedOut || 0) > 0) {
    p.batchSize    = Math.max(MIN_PACING.batchSize, p.batchSize - 1);
    p.batchDelay   = Math.max(MIN_PACING.batchDelay, Math.min(20_000, p.batchDelay + 1500));
    p.perSendDelay = Math.min(2500, Math.max(MIN_PACING.perSendDelay, p.perSendDelay + 150));
    return;
  }

  // Only speed up after a clean window of success.
  if (rate === 0 && (stats.sentOk || 0) >= 12) {
    p.batchSize    = Math.min(MAX_PACING.batchSize, p.batchSize + 1);
    p.batchDelay   = Math.max(MAX_PACING.batchDelay, p.batchDelay - 600);
    p.perSendDelay = Math.max(MAX_PACING.perSendDelay, p.perSendDelay - 80);
  }
}
...
function looksLikeThrottle(errMsg) {
  ...
  return (
    ...
    s.includes('temporar') ||
    s.includes('request_entity_too_large') ||
    s.includes('timeout') ||
    s.includes('socket') || s.includes('stream closed')
  );
}
...
/* --------------------- session preflight (critical) --------------------- */

// PARTICIPANT-AWARE: assert sessions for people, not the group JID
async function ensureSessionForJid(sock, jid) {
  const target = bareJid(jid); // strip :device

  if (target.endsWith('@g.us')) {
    try {
      const meta = await withTimeout(sock.groupMetadata(target), 8000, `meta:${target}`);
      const parts = (meta?.participants || []).map(p => bareJid(p?.id)).filter(Boolean);

      if (!parts.length) {
        await sleep(500);
        const meta2 = await withTimeout(sock.groupMetadata(target), 8000, `meta2:${target}`);
        (meta2?.participants || []).forEach(p => {
          const id = bareJid(p?.id);
          if (id) parts.push(id);
        });
      }

      if (typeof sock.assertSessions === 'function' && parts.length) {
        const CHUNK = 50;
        for (let i = 0; i < parts.length; i += CHUNK) {
          const chunk = parts.slice(i, i + CHUNK);
          try {
            await withTimeout(sock.assertSessions(chunk, false), 15000, `assertParts:${target}:${i}`);
          } catch {
            await sleep(400);
          }
          await sleep(150);
      ...
      // If metadata fetch fails, proceed; per-send path will still retry+backoff
    }
  } else {
    try {
      if (typeof sock.assertSessions === 'function') {
        await withTimeout(sock.assertSessions([target], false), 9000, `assert:${target}`);
      }
    } catch {}
  }

  // small human-ish delay
  await sleep(400 + Math.floor(Math.random() * 150));
}

/* --------------------------- media download (retry) --------------------- */
/** Avoids "Waiting for this message" by retrying + updateMediaMessage */
async function safeDownloadMedia(msg, sock, retries = 12, delay = 6000) {
  await sleep(1200); // let WA finalize upload + keys (↑)
  for (let i = 0; i < retries; i++) {
    try {
      ...
      if (!/cipher|not found|download media failed|message not available|waiting for this message|timed out/i.test(m)) {
        console.warn(`[media] non-retryable? ${m}`);
      }
    }
    if (i < retries - 1) await sleep(delay + Math.floor(Math.random() * 900));
  }
  return null;
}
...
async function sendToOneWithRetry(sock, jid, payload, u, onThrottleFn) {
  let attempt = 0;
  ...
  while (attempt <= SEND_MAX_RETRIES) {
    try {
      ...
      // If the socket actually died, bubble up so the upper layer can stop.
      if (/SOCKET_NOT_OPEN|Connection Closed/i.test(msg)) throw err;

      const isNoSessions = /no sessions?/i.test(msg);
      const retryable =
        isNoSessions ||
        /TIMEOUT:send|timed out|request_entity_too_large|temporar|retry|socket|stream|closed|too many/i.test(msg);

      attempt++;

      // Immediate global backoff on throttle-like signals
      if (looksLikeThrottle(msg) && typeof onThrottleFn === 'function') {
        await onThrottleFn(`send-level: ${msg.slice(0, 80)}`);
      }

      if (!retryable || attempt > SEND_MAX_RETRIES) throw err;

      if (isNoSessions) { try { await ensureSessionForJid(sock, jid); } catch {} }
 ...
 * - backoff + auto-resume on throttling (no hard aborts)
...
async function sendInBatches(sock, username, from, jids, messageContent) {
  ...
  // --- PREWARM: assert sessions for participants of each target group once ---
  try {
    await sendSys(username, sock, from, { text: `🔧 Preparing sessions for ${jids.length} group(s)…` });
    for (const gid of jids) {
      try { await ensureSessionForJid(sock, gid); } catch {}
      await sleep(120); // gentle cadence
    }
    await sendSys(username, sock, from, { text: `✅ Sessions ready. Starting broadcast…` });
  } catch {}
  ...
  async function onThrottle(reason) {
    ...
    await sleep(ms);
    // soften pacing after throttle
    p.batchSize   = Math.max(1, p.batchSize - 1);
  ...
  while (nextIndex < pendingQueue.length) {
    ...
    for (const jid of batch) {
      ...
      try {
        if (messageContent.text !== undefined) {
          ...
          await sendToOneWithRetry(sock, jid, { text }, u, onThrottle);
        } else if (imageBuffer) {
          const payload = {
            image: imageBuffer,
          ...
          await sendToOneWithRetry(sock, jid, payload, u, onThrottle);
        } else {
        ...
        // Also trigger throttle reaction immediately from batch level if it smells like throttle
        if (looksLikeThrottle(msg)) {
          await onThrottle(`batch-level: ${msg.slice(0, 80)}`);
        }
      }
    ...
    // Throttle signals → backoff & continue (no abort)
    ...
    if (failedThisBatch.length >= Math.ceil(batch.length / 2)) {
  ...
  // Late reattempt once for previously skipped JIDs after a global backoff, if any success happened.
  if (stats.sentOk > 0) {
    const hardSkipped = Array.from(badJids.entries()).filter(([, n]) => n >= 2).map(([jid]) => jid);
    if (hardSkipped.length) {
      await onThrottle('final reattempt window');
      // Clear counts and try once more, sequentially.
      for (const jid of hardSkipped) badJids.set(jid, 1);
      let pendingQueue2 = hardSkipped.slice();
      let nextIndex2 = 0;

      while (nextIndex2 < pendingQueue2.length) {
        const jid = pendingQueue2[nextIndex2++];
        try {
          if (messageContent.text !== undefined) {
            const text = withFooter(messageContent.text || '');
            await sendToOneWithRetry(sock, jid, { text }, u, onThrottle);
          ...
          stats.sentOk++; console.log(`[${username}] 🔁 Final attempt succeeded for ${jid}`);
        } catch (e) {
          console.warn(`[${username}] 🔁 Final attempt failed for ${jid}: ${e?.message || e}`);
        }
        await sleep(jitter(p.perSendDelay + 200));
      }
    }
  }
  ...
  if (skipped.length) {
    ...
    await sendSys(username, sock, from, {
      text: `ℹ️ Skipped ${skipped.length} group(s) after repeated failures:\n` +
...
async function handleBroadcastMessage(username, msg, sock) {
  ...
  // show the contents of a category
  ...
  if (selection && u.lastPromptChat === ownerBare && u.awaitingCategory && (u.pendingImage || u.pendingText)) {
    ...
    try {
      ...
      if (u.pendingImage) {
        ...
        await sendInBatches(sock, username, ownerBare, jids, { image: { url: imagePath }, caption: u.pendingImage.caption || '' });
  ...
  if (u.mode === 'media' && hasImage(msg)) {
    const buffer = await safeDownloadMedia(msg, sock, 12, 6000);
