...
function adjustPacing(u, stats) {
  ...
  if (rate >= 0.15 || (stats.timedOut || 0) > 0) {
    ...
    p.perSendDelay = Math.min(1000, Math.max(MIN_PACING.perSendDelay, p.perSendDelay + 50));
    ...
    p.perSendDelay = Math.max(MAX_PACING.perSendDelay, p.perSendDelay - 25);
  }
}
 ...
 * - pauses early if many timeouts → tells the user
...
async function sendInBatches(sock, username, from, jids, messageContent) {
  ...
  while (sent < total) {
    ...
    // early abort if clearly throttled at the start
    if (sent >= EARLY_ABORT_CHECK_AT && stats.timedOut >= Math.ceil(EARLY_ABORT_CHECK_AT * 0.4)) {
      const waitMins = 20;
      await sendSys(username, sock, from, {
        text: `⚠️ WhatsApp is throttling this account (many timeouts). Paused after ${sent} groups. Try again in ~${waitMins} minutes.`
      });
      break;
    }

    // batch-level throttle break (if half or more of this batch failed)
    const batchFail = batch.filter(j => (badJids.get(j) || 0) > 0).length;
    if (batchFail >= Math.ceil(batch.length / 2)) {
      await sendSys(username, sock, from, {
        text: `⚠️ Heavy throttling detected on this batch. Pausing to protect account. Try again in ~20 minutes.`
      });
      break;
    }

    if (sent < total) await sleep(p.batchDelay);
  }
