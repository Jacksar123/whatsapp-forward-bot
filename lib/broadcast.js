...
const {
  ...
  prepareWAMessageMedia, // ✅ function export from Baileys (not a sock method)
...
// Always append footer two lines *below* user content. If no content, just footer.
// Guard against double-tagging (if user forwards something with our footer already).
...
async function guardMemory512() {
  const mb = rssMB();
  // tuned for 512 MB: be aggressive
  if (mb > 440) {                  // ~85% of 512 MB
    await sleep(1000);             // give GC breathing room
  } else if (mb > 400) {
    await sleep(300);
  }
}

// Pre-upload once, free the original buffer/file immediately
async function prepareReusableImage(sock, imagePathOrBuffer) {
  let buf = Buffer.isBuffer(imagePathOrBuffer)
    ? imagePathOrBuffer
  ...
  // Free/zero the original bytes ASAP
  try { buf.fill(0); } catch {}
  buf = null;

  return prepared.image; // reuse this object for all sends
}

/* ---------------------- send-system wrapper (anti-echo) ----------------- */
// Track our own outgoing DM IDs and ignore them when they bounce back.
async function sendSys(USERS, username, sock, jid, content) {
  ...
  try {
    ...
    if (id) {
      u.ignoreIds.add(id);
      // Trim set if it grows too large (keep last ~200)
...
// ✅ 5-per-5s, but each batch sends its items **in parallel** (so a batch doesn't stretch to 25s)
// ✅ Media: pre-upload once and reuse (no memory spike)
async function sendInBatches(sock, username, from, jids, messageContent, USERS) {
  ...
  // If image, pre-upload once & unlink the temp file immediately
  let waImage = null;
  let baseCaption = messageContent.caption || '';
  let imagePathToUnlink = null;

  if (messageContent.image?.url) {
    imagePathToUnlink = messageContent.image.url;
    try {
      waImage = await prepareReusableImage(sock, imagePathToUnlink);
    } catch (e) {
      console.warn(`[${username}] pre-upload failed, falling back to per-send url: ${e?.message || e}`);
    } finally {
      // unlink the temp file right after pre-upload to keep memory/disk low
      try { fs.unlinkSync(imagePathToUnlink); } catch {}
      imagePathToUnlink = null;
    }
  }

  while (sent < total) {
    await guardMemory512(); // prevent batch start during high RSS

    const batch = jids.slice(sent, sent + BATCH_SIZE);
    const groupNames = batch.map(jid => USERS[username].allGroups[jid]?.name || jid);

    // Determine safe parallelism for THIS batch based on current RSS
    const mb = rssMB();
    const maxParallel = mb > 440 ? 3 : 5; // auto-drop parallelism if memory is tight

    // Run up to maxParallel concurrently within this batch
    let idx = 0;
    while (idx < batch.length) {
      ...
      await Promise.all(slice.map(async (jid) => {
        try {
          if (messageContent.text !== undefined) {
            ...
            // fallback path (slower): per-send url upload
            const caption = withFooter(baseCaption);
            await safeSend(jid, { image: { url: messageContent.image.url }, caption });
          } else {
      ...
      idx += slice.length;
      // micro-guard between slices of the same batch if memory is creeping up
    ...
    sent += batch.length;
    if (sent < total) await sleep(BATCH_DELAY_MS); // keep ban-safe pacing
  }
}
...
async function handleBroadcastMessage(username, msg, USERS) {
  ...
  if (selection && u.lastPromptChat === from && u.awaitingCategory && (u.pendingImage || u.pendingText)) {
    ...
    if (u.mode === 'text' && u.pendingText) {
      ...
      u.pendingText = null;
      u.awaitingPayload = 'text';   // re-arm to capture next text
      u.lastPromptChat = null;
      return;
    }
    ...
    if (u.pendingImage) {
      ...
      // ✅ Post-broadcast nudge for next image/text depending on current mode
      if (u.mode === 'text') {
        await sendSys(USERS, username, sock, from, {
          text: `✍️ Done. Send another message to broadcast, or /stop to cancel.`
        });
      ...
      // Cleanup & reset (image file already unlinked during pre-upload)
      u.pendingImage = null;
      u.lastPromptChat = null;
      return;
    }
