...
async function handleBroadcastMessage(username, msg, USERS) {
  ...
  // (Baileys can emit upserts multiple times as the message syncs)
  ...
  if (/^\/mode\s+(text|media)$/i.test(body)) {
    ...
    return await sock.sendMessage(from, {
      text: `✅ Mode changed. You are now in *${u.mode}* mode.\n` +
            `(use /${u.mode === 'text' ? 'media' : 'text'} to switch back)`
    });
  }
  if (body === '/text') {
    u.mode = 'text';
  ...
  // IMPORTANT: only capture new text if we're NOT already awaiting a category
  if (u.mode === 'text' && !u.awaitingCategory && body && !body.startsWith('/')) {
    u.pendingText = body;
    u.pendingImage = null;
  ...
  // If we ARE awaiting a category and user sends non-numeric junk, ignore (no spam)
  if (u.awaitingCategory && !selection) {
    return; // silently drop to avoid repeated "choose category" prompts
  }

  // Media mode: if an image arrives, capture file to disk and prompt
