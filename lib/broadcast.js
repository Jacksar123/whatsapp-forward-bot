// lib/broadcast.js
// Message-first → then pick category.
...
// Show a per-batch preview ("Sending batch ...") before each batch
const SHOW_BATCH_PREVIEW = (process.env.SHOW_BATCH_PREVIEW || 'true').toLowerCase() !== 'false';
...
// Recent system messages echo filter (to avoid rebroadcasting prompts)
function markSysEcho(username, text) {
  if (!text) return;
  const u = USERSG()[username] || (USERSG()[username] = {});
  if (!u._recentSys) u._recentSys = [];
  const now = Date.now();
  u._recentSys.push({ t: now, text: String(text) });
  // keep last 20, prune > 2 minutes
  u._recentSys = u._recentSys
    .slice(-20)
    .filter(e => now - e.t < 2 * 60 * 1000);
}
function isRecentSysEcho(u, body) {
  if (!u?._recentSys || !body) return false;
  const now = Date.now();
  // prune old
  u._recentSys = u._recentSys.filter(e => now - e.t < 2 * 60 * 1000);
  return u._recentSys.some(e => e.text === body);
}

// Single place to cancel any active flow (text/media/category/broadcast)
function cancelFlow(u) {
  requestCancel(u);
...
async function sendSys(username, sock, jid, content) {
  ...
  try {
    ...
    // Record selection/category/system prompts to ignore their echo later
    if (content && typeof content.text === 'string') {
      markSysEcho(username, content.text);
    }
    return res;
  } catch (e) {
...
function isSelectionPromptText(t) {
  const s = String(t || '').toLowerCase();
  return s.startsWith('✅ selected'.toLowerCase()) ||
         s.includes('now type your message and send');
}
...
async function sendInBatches(sock, username, from, jids, messageContent, jobId) {
  ...
  // Block accidental system-prompt broadcast
  if (messageContent.text && isSelectionPromptText(messageContent.text)) {
    await sendSys(username, sock, from, { text: '⚠️ Ignored system prompt; waiting for your actual message.' }).catch(()=>{});
    return;
  }
  ...
  const waitSecs = Math.max(1, Math.round(BATCH_INTERVAL_MS / 1000));
  ...
  for (let batchStart = 0; batchStart < jids.length; batchStart += BATCH_SIZE) {
    ...
    // NEW: announce batch with group names
    if (SHOW_BATCH_PREVIEW) {
      const names = batch.map(id => (u?.allGroups?.[id]?.name || id)).filter(Boolean);
      const bullets = names.map(n => `• ${n}`).join('\n');
      const header = `📤 Sending batch ${batchNum}/${totalBatches} (${batch.length} group${batch.length>1?'s':''}):`;
      await sendSys(username, sock, from, { text: `${header}\n${bullets}` }).catch(()=>{});
    }
    ...
    // NEW: wait message between batches
    const moreLeft = (batchStart + BATCH_SIZE) < jids.length;
    if (moreLeft) {
      await sendSys(username, sock, from, { text: `⏳ Please wait ${waitSecs}s before next batch…` }).catch(()=>{});
...
function extractNumericChoice(text) {
  ...
  // Accept "2", "2.", "2)", "2 -", "2 something"
  const m = t.match(/^(\d{1,3})\b/);
  if (m) return parseInt(m[1], 10);

  // Accept keycap emoji like "2️⃣"
...
async function handleBroadcastMessage(username, msg, sock) {
  ...
  // Ignore self system echoes, BUT allow commands anytime and digits during selection
  if (fromMe) {
    if (!body) return;
    ...
    if (!body.startsWith('/')) {
      ...
      if (sysPhrases.some(t => body.includes(t))) return;
      if (isRecentSysEcho(u, body)) return; // extra guard

      // While waiting for a category, only let numeric replies pass through
      if (u.awaitingCategory) {
        const n = extractNumericChoice(body);
        if (n == null) return; // block non-numeric chatter
      }
    }
  }
  ...
  if (body === '/help') {
    cancelFlow(u); // cancel any in-progress flow first
    const help = [
      '✅ *Commands*',
      '• /help — this help',
  ...
  if (body === '/text') {
    cancelFlow(u);         // ensure clean switch
    u.mode = 'text';
  ...
  if (maybeNum && u.lastPromptChat === chatJid && u.awaitingCategory) {
    ...
    // Allow the next user message to be the broadcast text (do NOT send anything yet)
    u.selectedTarget = { type: chosen === '__ALL__' ? 'all' : 'category', key: chosen };
    u.awaitingPayload = 'text';
    u.awaitingCategory = false; // important to let next message through
    if (u.categoryTimeout) { clearTimeout(u.categoryTimeout); u.categoryTimeout = null; }
  ...
  if (u.mode === 'text' && body && !body.startsWith('/' ) && !u.awaitingCategory) {
    ...
    // Guard: ignore our own selection prompt echo
    if (
      body.startsWith('✅ Selected') &&
      body.includes('Now type your message and send.')
    ) {
      return; // ignore echo line, don't set pendingText
    }
    if (isRecentSysEcho(u, body)) return; // extra guard
