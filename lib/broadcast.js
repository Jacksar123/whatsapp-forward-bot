...
// Pace: 5 groups per batch, wait 5s between batches
const BATCH_SIZE = 5;
const PARALLEL_PER_SLICE = 5;         // up to 5 in parallel within a batch
const PER_SEND_DELAY_MS = 200;        // tiny spacing inside the slice
const BATCH_DELAY_MS = 5000;          // 5 seconds between batches

// Robustness
const SEND_MAX_RETRIES = 3;           // retry per group send
const SEND_BACKOFF_MS = 1200;         // base backoff for retries
...
/**
 * Avoids "Waiting for this message" by:
 *  - adding an initial small delay,
 *  - retrying many times with exponential backoff,
 *  - using updateMediaMessage so WA re-uploads media to the CDN.
 */
async function safeDownloadMedia(msg, sock, retries = 12, initialDelay = 1500) {
  // initial wait so the sender finishes uploading to CDN
  if (initialDelay > 0) await sleep(initialDelay);

  let delay = 1200; // start backoff
  for (let i = 0; i < retries; i++) {
    try {
      ...
      console.warn(`[media] download attempt ${i + 1}/${retries} failed: ${m}`);
    }
    // next try
    await sleep(delay);
    delay = Math.min(Math.floor(delay * 1.6) + 250, 6000);
  }
  return null;
}
...
async function sendToOneWithRetry(sock, jid, payload) {
  ...
  while (attempt <= SEND_MAX_RETRIES) {
    try {
      const res = await (sock.safeSend ? sock.safeSend(jid, payload) : sock.sendMessage(jid, payload));
      return res;
    } catch (err) {
      const msg = String(err?.message || err);
      const retryable =
        /timed out|rate|temporar|retry|disconnect|socket|stream|closed|request_entity_too_large|too many|econn|network/i
          .test(msg);
      attempt++;
...
async function sendInBatches(sock, username, from, jids, messageContent) {
  ...
  if (messageContent.image?.url) {
    ...
    try {
      ...
      console.log(`[${username}] media present=true size=${stat.size}B — sending bytes to each group`);
    } catch (e) {
...
async function handleBroadcastMessage(username, msg, sock) {
  ...
  if (u.mode === 'text') {
    ...
    if (u.awaitingCategory && !extractNumericChoice(msg)) return;
  }

  if (u.mode === 'media' && hasImage(msg)) {
    // Try download with robust helper
    let buffer = await safeDownloadMedia(msg, sock, 12, 1500);

    // If still not ready, do one final delayed retry to defeat "waiting for this message"
    if (!buffer?.length) {
      await sleep(4000);
      buffer = await safeDownloadMedia(msg, sock, 6, 0);
    }

    if (!buffer?.length) {
      await sendSys(username, sock, ownerBare, {
        text: '❌ Failed to download image (media not ready yet). Please resend the image.'
      });
      return;
    }
