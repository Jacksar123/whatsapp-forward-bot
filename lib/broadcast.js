...
/**
 * Pre-upload once using prepareWAMessageMedia.
 * IMPORTANT: When sending later, use `{ image: prepared.imageMessage, caption }`
 */
async function preUploadImage(sock, imagePathOrBuffer) {
  ...
  if (!prepared?.imageMessage) {
    throw new Error('prepareWAMessageMedia returned no imageMessage');
  }
...
// 5-per-5s; inside each batch, send in parallel
async function sendInBatches(sock, username, from, jids, messageContent, USERS) {
  ...
  // If image, try pre-upload once
  let waMediaPrepared = null; // { imageMessage: ... }
  let baseCaption = messageContent.caption || '';
  let imagePath = null;       // keep a copy for fallback
  let canDeleteAtEnd = false; // only delete file after all sends if we needed fallback

  if (messageContent.image?.url) {
    imagePath = messageContent.image.url;

    try {
      // Pre-upload (best path): read file → upload → delete immediately
      const buf = fs.readFileSync(imagePath);
      waMediaPrepared = await prepareWAMessageMedia({ image: buf }, { upload: sock.waUploadToServer });
      if (!waMediaPrepared?.imageMessage) {
        throw new Error('prepareWAMessageMedia returned no imageMessage');
      }
      try { buf.fill(0); } catch {}
      // file no longer needed after successful pre-upload
      try { fs.unlinkSync(imagePath); } catch {}
      imagePath = null;
    } catch (e) {
      // Pre-upload failed — keep the file for per-send fallback; delete at the very end
      canDeleteAtEnd = true;
      console.warn(`[${username}] pre-upload failed, will fallback to per-send file path: ${e?.message || e}`);
    }

    // Debug: confirm file presence if we didn't pre-upload
    try {
      const stat = imagePath && fs.existsSync(imagePath) ? fs.statSync(imagePath) : null;
      console.log(`[${username}] media file present=${!!stat} size=${stat ? stat.size : 0}B preUploaded=${!!waMediaPrepared}`);
    } catch (e) {
  ...
  while (sent < total) {
      ...
      await Promise.all(slice.map(async (jid) => {
        try {
          if (messageContent.text !== undefined) {
            ...
            // ✅ Correct way: embed prepared payload under `image`
            const caption = withFooter(baseCaption);
            await safeSend(jid, {
              image: waMediaPrepared.imageMessage, // <-- key fix to avoid blank media
              caption,
              contextInfo: { forwardingScore: 2, isForwarded: true }
            });
          } else if (imagePath) {
            // Fallback path: use on-disk file (must still exist)
            if (!fs.existsSync(imagePath)) {
              throw new Error(`Image file missing before send: ${imagePath}`);
            }
            const caption = withFooter(baseCaption);
            await safeSend(jid, { image: { url: imagePath }, caption }); // no manual mimetype
          } else {
    ...
    sent += batch.length;
    if (sent < total) await sleep(theBATCH_DELAY_MS);
  }

  // If we fell back to per-send file path, clean it up now that we're done
  if (canDeleteAtEnd && imagePath) {
    try { fs.unlinkSync(imagePath); } catch {}
  }
}
...
async function handleBroadcastMessage(username, msg, USERS) {
  ...
  if (selection && u.lastPromptChat === from && u.awaitingCategory && (u.pendingImage || u.pendingText)) {
    ...
    if (u.pendingImage) {
      ...
      // Cleanup & reset (actual file cleanup handled inside sendInBatches)
      u.pendingImage = null;
      u.lastPromptChat = null;
      return;
    }
  ...
  if (m.imageMessage && u.mode === 'media') {
    ...
    const imagePath = path.join(userPaths.tmp, `image_${timestamp}.jpg`);
